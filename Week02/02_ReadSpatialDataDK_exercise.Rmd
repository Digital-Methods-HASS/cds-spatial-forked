...
1. How many features (shapes *polygon, point, line*) does each layer (*each dataset is a layer*) contain and what kind of geometry are they? == How many observations in each dataobject (data object is a collection of one or more data points that create meaning as a whole) ---> soo how many rows? 
**Forests** 
layers: 32, since there are 32 rows?
geometry: polygons (MULTIPOLYGON)
...
st_crs(forests)
st_crs(parks)
#the playgrounds does not work? -> there are no CRS, person who send it didn't attach that...
st_crs(playgrounds)
#define the crs to the same ESPG as in the other files!
st_crs(playgrounds) <- 25832
#run the st_crs(playgrounds) again, and it will work! 
st_crs(shelters)
...
parks5ha <- filter(parks, areal_m2 > 5000)
#but this is problematic! you cannot be sure that people type in the right information, extract the data from the polygons instead
parks$areaofgeom <- st_area(parks)

over5lim <- parks %>% filter(unclass(areaofgeom) > 5000)
...
over5_lim <- parks5ha %>% select(navn, areal_m2) 

# Use head() to look at the first few records
head(over5_lim)

# the reason why the geometry column is not deleted is due to the fact that it is ingrained in the CRS, and so to speak not really a data column, but same kind of weird format as in parks areaofgeoms but same kind of  but a data column attached to the projected CRS part of the file (remember, it is not a singular file but a multiple file)
```
